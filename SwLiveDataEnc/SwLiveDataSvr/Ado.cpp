...
BOOL CADODatabase::Execute(LPCTSTR lpstrExec, CString & strMsg)
{
	ASSERT(m_pConnection != NULL);
	ASSERT(strcmp(lpstrExec, _T("")) != 0);
	_variant_t vRecords;
	...
	catch(_com_error &e)
	{
		...
		_bstr_t bstrSource(e.Source());
		_bstr_t bstrDescription(e.Description());
		strMsg.Format( "CADODataBase Error\n\tCode = %08lx\n\tCode meaning = %s\n\tSource = %s\n\tDescription = %s\n",
			e.Error(), e.ErrorMessage(), (LPCSTR)bstrSource, (LPCSTR)bstrDescription);
...
BOOL CADOParameter::GetValue(double& dbValue)
{
	...
	try
	{
		...
		case VT_DECIMAL:
			...
			dblVal /= pow(10.0f, vtVal.decVal.scale); 
			break;
		case VT_UI1:
			dblVal = vtVal.iVal;
			break;
...
BOOL CADOParameter::GetValue(CString& strValue, CString strDateFormat)
{
	...
	try
	{
		...
		case VT_DECIMAL:
			...
			val /= pow(10.0f, vtVal.decVal.scale); 
			strVal = DblToStr(val);
			}
			break;
		case VT_DATE:
